...
const ReturnsPage = () => {
    ...
    if (returns.length === 0) {
        return (
            <div className="bg-background min-h-screen py-12 flex flex-col items-center justify-center text-center px-4">
                <div className="w-20 h-20 bg-gray-100 rounded-full flex items-center justify-center mb-6 text-gray-400">
                    <RefreshCw size={32} />
                </div>
                <h2 className="text-2xl font-black text-textPrimary mb-2">No Returns Yet</h2>
                <p className="text-gray-500 mb-8 max-w-sm">
                    You haven't placed any return or replacement requests yet.
                </p>
                <Link to="/orders" className="bg-secondary text-white px-8 py-3 rounded-xl font-bold hover:bg-opacity-90 transition-all shadow-lg">
                    Go to My Orders
                </Link>
            </div>
        );
        ...
        <div className="bg-background min-h-screen py-4 md:py-12">
            <div className="container mx-auto px-3 md:px-12">
                <div className="flex items-center gap-2 md:gap-4 mb-6 md:mb-10">
                    <button onClick={() => navigate('/orders')} className="p-2 -ml-2 hover:bg-gray-100 rounded-lg transition-colors text-textPrimary/70">
                        <ArrowLeft size={20} md:size={24} />
                    </button>
                    <div>
                        <h1 className="text-xl md:text-3xl font-black text-textPrimary uppercase tracking-tighter md:tracking-tight leading-none">Returns</h1>
                                ...
                                <div className="flex flex-col gap-3">
                                    ...
                                    <div className="flex justify-between items-start">
                                        <div className="space-y-1">
                                            <div className="flex items-center gap-2">
                                                <span className="font-mono font-black text-textPrimary text-sm md:text-lg tracking-tight">{request.id}</span>
                                    ...
                                    <div className="pt-1 flex flex-col gap-3">
                                        <button
                                            ...
                                            className="w-full md:w-fit md:ml-auto md:px-10 bg-slate-50 border border-slate-100 text-textPrimary py-2.5 md:py-3 rounded-xl text-[9px] md:text-xs font-black uppercase tracking-widest active:scale-95 transition-all flex items-center justify-center gap-2 group hover:bg-secondary hover:text-white"
                                        >
                                            View Status
                                        ...
                                        {request.status === 'Pending' && (
                                            <div className="flex gap-2 items-center text-[8px] md:text-[10px] text-slate-300 font-bold uppercase tracking-tight justify-center md:justify-end">
                                                <AlertCircle size={10} className="text-primary/80 shrink-0" />
