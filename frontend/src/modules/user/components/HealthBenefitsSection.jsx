...
const HealthBenefitsSection = ({ data }) => {
    ...
    const cleanedTitle = (sectionData.title || 'Health Benefits').replace(/\s*Section\s*/gi, ' ').trim();
            ...
            <div className="container mx-auto px-2 md:px-4 max-w-screen-2xl relative z-10">
                ...
                <div className="text-center mb-8 md:mb-14 space-y-2 md:space-y-3">
                    <motion.h2
                        ...
                        viewport={{ once: true }}
                        className="text-2xl md:text-4xl font-['Poppins'] font-bold text-textPrimary tracking-tight leading-tight"
                    >
                        {titlePrefix ? `${titlePrefix} ` : ''}<span className="text-secondary">{lastWord}</span>
                    </motion.h2>
